        ...
        <div class="row">
            <div class="col-md-6 ms-auto text-center mt-3 ">
                ...
                @if(!empty($users[$note->admin_id]->photo))
                    <a href="javascript:" class=" mt-4 avatar avatar-md rounded-circle border border-secondary">
                        <img alt="" class="p-1" src="{{PUBLIC_DIR}}/uploads/{{$users[$note->admin_id]->photo}}">
                    </a>
                @else
                    <div class="avatar avatar-md  mt-4 rounded-circle bg-purple-light  border-radius-md p-2">
                ...
                <p>{{$note->created_at->format(config('app.date_format'))}}</p>
            </div>
            <div class="col-md-6">
