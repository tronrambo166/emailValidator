{{ title_render::Berevi Collection - Optimail Cleaner Table }}
...
{{ style_render:: }}
		...
		<link rel="stylesheet" href="{{ app_path }}/css/dataTables.bootstrap4.min.css">
		<!-- Zebra dialog -->
...
{{ script_body_render:: }}
		...
		<!-- Custom script -->
		<script type="text/javascript" src="{{ app_path }}/js/oc-tables.js"></script>
		<!-- Script -->
		...
		$(document).ready(function() {
			if ($('#isRecords').css('display') == 'block') {
			    var table = $('#ocTable').DataTable({
				    ...
				    "ajax": {
				        'type': 'POST',
				        'url': '{{ app_path }}/admin/panel/optimail/tables/objects',
				    },
				    "autoWidth": false,
				    "deferRender": true,
					"columnDefs": [
			    ...
			    $('#ocTable tbody').on('click', 'td.details-control', function() {
			    ...
			    buttons: [
			        {
			        	...
			        	callback: function() {
						   	setTimeout(function () {
						       	window.location.replace("{{ app_path }}/admin/panel/optimail/tables/delete/files/" + res);
						    }, 500);
			        	}
			        },
			        {
		...
		function downloadGood(elem) {
			var id = $(elem).attr("id");
	    	var res = id.split("-", 1);
			$.Zebra_Dialog('Preparing the <strong>"Good"</strong> file for download. Please wait...', {
			    type: 'confirmation',
			    ...
			    onClose: function(caption) {
				   	setTimeout(function () {
				       	window.location.replace("{{ app_path }}/admin/panel/optimail/tables/download/good/" + res);
				    }, 500);
			    }
			});
		}
		...
		function downloadBad(elem) {
			...
			$.Zebra_Dialog('Preparing the <strong>"Bad"</strong> file for download. Please wait...', {
			    ...
			    onClose: function(caption) {
				   	setTimeout(function () {
				       	window.location.replace("{{ app_path }}/admin/panel/optimail/tables/download/bad/" + res);
				    }, 500);
			    }
			});
		}
...
{{ @include::BereviCollectionPack:Main:adminPanel.php }}
				...
				<div class="jumbotron masthead-page">
					...
					<div class="alert alert-primary" role="alert" style="{{ isRecordsMsg }}">
					  	...
					  	<p>Aww yeah, you have not started cleaning any email addresses yet and thereby do not have records of available.</p>
					  	<hr>
					 	<p class="mb-0">Whenever you need to, be sure to use the cleaning process before consulting your records.</p>
					</div>
					<div id="isRecords" class="alert alert-light" role="alert" style="{{ isRecords }}">
	                    <table id="ocTable" class="table table-striped table-bordered" cellspacing="0" width="100%">
					        <thead>
					            <tr>
					                <th></th>
					                <th>id</th>
